...
for log in log_files:
    with open(log, 'r') as fp:
        ...
        for row in reader:
            ...
            elif row[0] == "memoryLens":
                if flag:
                    memlen_dist.append([float(i) for i in row[1:]])
...
def get_binned_values(X, Y):
    freq = dict()
    for d in set(X):
        choice = [Y[i] for i in range(len(X)) if X[i] == d]
        freq[d] = np.mean(choice)
    return freq


# plot of age distribution in the population
for idx in range(len(age_dist)):
    # plt.plot(list(range(len(i))), sorted(i), label="Memory sharing = " + str(x[idx]))
    ages = age_dist[idx]
    mem_coop = memlen_dist[idx]
    mem_lens = [round(i) for i in mem_coop]
    age_binned = get_binned_values(mem_lens, ages)
    plt.bar(age_binned.keys(), age_binned.values())
    plt.title("Memory sharing = %d" %(x[idx]))
    plt.xlabel("Length of memory")
    plt.ylabel("Average number of timesteps the agents survived")
    plt.legend()
    plt.savefig(img_store_dir + "/age_distribution_%d.png" %(x[idx]))
    # plt.show()
    plt.gcf().clear()
